...
TAG_ID = 'E280-1160-6000-0209-F811-72E3'
PHASE_LOAD_NUM = 2
TAG_SIZE = 0.16
# antenna params
NUM_ANTENNA = 2
    ...
    def callback_2(self, msg):
        ...
        if msg.epc == TAG_ID:
            ...
            if self.state_status == 0:
                ...
                # maybe not go staright, should go for a circle which can cover all the area
                ...
                # if (self.rssi_list[0] != None and abs(self.rssi_list[0]) < 50) or (self.rssi_list[1] != None and abs(self.rssi_list[1]) < 50):
                #     self.twist_msg.linear.x = 0.0
                #     self.twist_msg.angular.z = 0.0
                #     self.publisher.publish(self.twist_msg)
                #     self.state_status = 1
                ...
                ## new version ##
                # but still ignore the single antenna case
                self.rssi_list[msg.ant - 1] = msg.rssi
                
                self.twist_msg.linear.x = 0.04

                if self.rssi_list[0] != None and self.rssi_list[1] != None:
                    if abs(self.rssi_list[0]) < 53 and abs(self.rssi_list[1]) < 53:
                        self.twist_msg.linear.x = 0.0
                        self.twist_msg.angular.z = 0.0
                        # self.publisher.publish(self.twist_msg)
                        self.state_status = 1
                        self.begin_time = msg.time # used for buffer

                    if self.rssi_list[0] < self.rssi_list[1] - 2:
                        self.twist_msg.angular.z = 0.1
                        # print('turn left')
                        # print(self.rssi_list)
                    elif self.rssi_list[0] - 2 > self.rssi_list[1]:
                        self.twist_msg.angular.z = -0.1
                        # print('turn right')
                        # print(self.rssi_list)
                    else:
                ...
                ## 55 for 2m, 45 for 1m
                # if (self.rssi_list[0] != None and abs(self.rssi_list[0]) < 50) and (self.rssi_list[1] != None and abs(self.rssi_list[1]) < 50):
                    # self.twist_msg.linear.x = 0.0
                    # self.twist_msg.angular.z = 0.0
                    # self.publisher.publish(self.twist_msg)
                    # self.state_status = 1
                ...
                print('-----state 1-----') # adjust the facing direction until rssi from ant1 and ant2 is approximate

                self.rssi_list[msg.ant - 1] = msg.rssi
                print(self.rssi_list)

                if self.rssi_list[0] != None and self.rssi_list[1] != None: # what is used for?
                    if (abs(self.rssi_list[0] - self.rssi_list[1]) < 0.5) and (msg.time - self.begin_time > 3 * 1000000):
                        # self.twist_msg.linear.x = 0.0
                        # self.twist_msg.angular.z = 0.0
                        # self.publisher.publish(self.twist_msg)
                        self.state_status = 2

                        # if abs(self.rssi_list[0]) < 50 and abs(self.rssi_list[1]) < 50:
                        #     self.state_status = 2
                        #     self.twist_msg.linear.x = 0.0
                        # else:
                        #     self.twist_msg.linear.x = 0.02 # 0.04

                    elif self.rssi_list[0] < self.rssi_list[1] - 0.5:
                        self.twist_msg.angular.z = 0.1
                        # print('turn left')
                    elif self.rssi_list[0] - 0.5 > self.rssi_list[1]:
                        self.twist_msg.angular.z = -0.1
                        # print('turn right')
                ...
                if self.flag:
                    self.begin_time = msg.time # used as timer
                    self.begin_rssi = self.rssi_list[0] # used to determine distance used for localization
                    self.flag = False
                ...
                self.twist_msg.linear.x = 0.06
                self.twist_msg.angular.z = 0.0
                if msg.ant == 2:
                    self.publisher.publish(self.twist_msg)
                    # record the phase data
                    # phase_dist_writer.writerow([self.phasor_unwrapped[0], self.phasor_unwrapped[1]])

                # linear speed 0.02, 54s for 1m
                # linear speed 0.06, 17.8s for 1m, 8.9s for 0.5m
                if msg.time - self.begin_time > 17.5 * 1000000:
                    self.twist_msg.linear.x = 0.0
                    self.twist_msg.angular.z = 0.0
                    self.publisher.publish(self.twist_msg)
                    self.state_status = 3
                    ...
                    var_ph_ant2 = (self.begin_phasor[1] - self.phasor_unwrapped[1]) * WAVE_LENGTH / (4 * np.pi)
                    ...
                    for _ in range(751):
                        ...
                        val2 = ((x+0.375)**2) * ((var_ph_ant2/2)**2) / (((dist/2)**2) - ((var_ph_ant2/2)**2)) + ((var_ph_ant2/2)**2)
                        val_temp = abs(val1 - val2)
                    ...
                    print('ant2: ', var_ph_ant2)
                    print('min_val: ', min(val_result))
                    # print(val_result)
                ...
                if self.phasor_unwrapped[msg.ant - 1] == None:
                    ...
                    if msg.ant == 2:
                        ...
                        if True:
                            ...
                            ### solving triangle using ant 1 and 2, in 2D version x-y plane
                            tag_antref_position = solve_triangle(1, 4, self.phasor_unwrapped[1] * WAVE_LENGTH / (4 * np.pi), self.phasor_unwrapped[0] * WAVE_LENGTH / (4 * np.pi))
